...
describe('generateLut', function() {
    it('min pixel < 0', function() {
        ...
        // Act
        var lut = generateLut(image, 2, 0, false);
    ...
    it('min pixel = 0', function() {
        ...
        // Act
        var lut = generateLut(image, 2, 1, false);
    ...
    it("min pixel > 0", function() {
        ...
        // Act
        var lut = generateLut(image, 2, 2, false);
    ...
    it("slope and intercept", function() {
        ...
        // Act
        var lut = generateLut(image, 4, 3, false);
    ...
    it("should handle inverted images correctly", function() {
        ...
        // Act
        var lut = generateLut(image, 4, 3, true);

        // Assert
        assert.equal(lut.length, 3, "lut length is 3");
        assert.equal(lut[0], 0, "lut entry 0 is 255");
        assert.equal(lut[1], 128, "lut entry 1 is 128");
        assert.equal(lut[2], 255, "lut entry 2 is 0");
    });
});