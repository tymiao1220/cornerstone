...
function getShaderProgram(image) {
    ...
    var shader = shaders.rgb;
    return shader;
}

function getImageTexture( image ) {
    var imageTexture = textureCache.getImageTexture(image.imageId);
    if (!imageTexture) {
...
function generateTexture( image ) {
    ...
    var imageData = dataUtilities[imageDataType].storedPixelDataToImageData(image, image.width, image.height);
